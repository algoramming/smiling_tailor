                  ...
                  children: [
                    Consumer(builder: (_, ref, __) {
                      ...
                      final noti = ref.watch(
                          vendorTrxsProvider(notifier.selectedVendor!)
                              .notifier);
                      return Expanded(
                        child: TextFormField(
...
class _TrxList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return Expanded(
      child: ref.watch(vendorTrxsProvider(notifier.selectedVendor!)).when(
          loading: () => const LoadingWidget(withScaffold: false),
          error: (err, _) => KErrorWidget(error: err),
          data: (_) {
            final noti = ref
                .watch(vendorTrxsProvider(notifier.selectedVendor!).notifier);
